...
#         # --- f(s) branch: 현재 상태 s를 임베딩 ---
#         self.f_conv1 = nn.Conv2d(in_channels, hidden_dim, kernel_size=3, padding=1)
#         # self.f_bn1   = nn.BatchNorm2d(hidden_dim)
#         # self.f_res = ResidualBlock(channels=channel)
#         self.f_fc1   = nn.Linear(hidden_dim  * state_dim, hidden_dim)
...
#     def __init__(self, embed_dim, num_heads, dropout=0.1, ff_hidden_dim=None):
#         """
#         Args:
#             embed_dim: 임베딩 차원
...
#     model = Net(num_transformer_layers=3, dropout=0.1)
#     B, H, W = 2, 3, 3
#     s = torch.randn(B, 3, H, W)
    
#     # 어텐션 맵을 반환하도록 호출
#     policy, value, attn_maps = model(s, return_attention=True)
    
#     print("Policy logits shape:", policy.shape)  # (B, 9)
#     print("Value shape:", value.shape)             # (B, 1)
    
#     # 예를 들어, 첫 번째 Transformer 레이어의 어텐션 맵 확인
#     print("First layer attention map shape:", attn_maps[0].shape)
import torch
import torch.nn as nn
import torch.nn.functional as F
import torch.nn.utils.rnn as rnn_utils
...
class Net(nn.Module):
    def __init__(self, img_size, patch_size, embed_dim, action_dim, num_heads, depth):
        super().__init__()
        ...
        self.policy_fc = nn.Linear(embed_dim, img_size * img_size * 2)
        self.value_fc = nn.Linear(embed_dim, img_size * img_size)
        self.policy_head = nn.Linear(img_size * img_size * 2, action_dim)
        self.value_head = nn.Linear(img_size * img_size, 1)
